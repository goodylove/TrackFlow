...
export function LandingFooter() {
  return <footer className="bg-white pb-10 pt-2 sm:pb-12" id="footer">
    <Container><Reveal direction="right"><div className="grid gap-10 border-b border-[var(--marketing-border)] pb-10 lg:grid-cols-[1.4fr_repeat(4,1fr)]"><div><Link to="/" className="text-sm font-black text-[var(--marketing-action)]">TrackFlow</Link><p className="mt-4 max-w-[14rem] text-[0.82rem] leading-6 text-[var(--marketing-muted-foreground)]">Issue tracking for small teams. One place for the work, the owner, and the conversation.</p></div>{footerGroups.map(([heading, links]) => <div key={heading}><h3 className="text-[0.78rem] font-black text-[var(--marketing-foreground)]">{heading}</h3><div className="mt-4 space-y-3">{links.map((label) => {
