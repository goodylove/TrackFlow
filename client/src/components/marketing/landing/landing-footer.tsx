...
export function LandingFooter() {
  return <footer className="bg-white pb-12 pt-8" id="footer">
    <Container><Reveal direction="right"><div className="grid gap-10 border-b border-[var(--marketing-border)] pb-10 lg:grid-cols-[1.4fr_repeat(4,1fr)]"><div><a href="/" className="text-sm font-black text-[var(--marketing-action)]">TrackFlow</a><p className="mt-4 max-w-[14rem] text-[0.82rem] leading-6 text-[var(--marketing-muted-foreground)]">Helping teams stay on track, collaborate seamlessly, and resolve issues effectively.</p></div>{footerGroups.map(([heading, links]) => <div key={heading}><h3 className="text-[0.78rem] font-black text-[#171722]">{heading}</h3><div className="mt-4 space-y-3">{links.map((label) => {
      ...
      if (href.startsWith("#")) return <a key={label} className={className} href={href}>{label}</a>
